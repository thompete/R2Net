...
public class NetworkNode {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                case "-ident" -> id = Integer.parseInt(args[++i]);
                ...
                default -> {
                    if (resources == null) resources = new StringBuilder(args[i]);
                    else resources.append(" ").append(args[i]);
                }
            }
        }

        new NetworkNode(id, port, gatewayIp, gatewayPort, resources.toString());
    }
    ...
    private NetworkNode(int id, int port, String gatewayIp, int gatewayPort, String resources) throws Exception {
        ...
        this.resources = convertResourceStringToMap(resources);
    ...
    private List<NodeData> readNodes(BufferedReader in) throws IOException {
    ...
    private List<NodeData> sendGetNetworkRequest(String ip, int port) throws Exception {
        ...
        List<NodeData> network = readNodes(in);
    ...
    private Map<Character, Integer> convertResourceStringToMap(String resourceStr) {
    ...
    private List<String> allocateResources(ClientData client, List<NodeData> checkedNodes) throws Exception {
        ...
        for (Character key : client.requestedResources.keySet()) {
            int requestedAmount = client.requestedResources.get(key);
            if (resources.containsKey(key)) {
                int amount = resources.get(key);
                if (amount >= requestedAmount) {
                    handledResources.put(key, requestedAmount);
                } else {
                    int rest = requestedAmount - amount;
                    if (amount > 0) handledResources.put(key, amount);
                    unhandledResources.put(key, rest);
                }
            } else {
                unhandledResources.put(key, requestedAmount);
            }
        }
    ...
    private class RequestHandler extends Thread {
        ...
        private void handleAllocateRequest() throws Exception {
            ...
            List<NodeData> checkedNodes = readNodes(in);
    ...
    private class ClientData {
        ...
        public ClientData(int id, String ip, int port, String requestedResources) {
            this(id, ip, port, convertResourceStringToMap(requestedResources));
        }

        @Override
        public boolean equals(Object o) {
            if (this == o) return true;
            if (!(o instanceof ClientData other)) return false;
            return id == other.id;
        }
